...
//apikey
const apivhtear = 'GithubZero' // Vhtear Beli Sendiri Yoo
const ZeksApi = 'apivinz'
const apileys = 'OneDayOneCharity' // Free Apikey
const TobzKey = 'BotWeA'
const lolhuman = 'ZeroAPI' //Besok Beli Sendiri
const onlydev = 'DarkApi108' //Free Apikey
const xteam = 'ZeroBotWa' //Hari Jumat Habis

//Ubah
const namaowner = 'Zero'
...
const Zero = new WAConnection()
   ...
   console.log(color('[','aqua'),color('Zero','white'),color(']','aqua'),color('SQAN QR CODE DI WHATSAPP WEB!!','aqua'),color('You','white'),color('Tube','red'),color('Zero','yellow'))
}) 

Zero.on('credentials-updated', () => {
	const authInfo = Zero.base64EncodedAuthInfo()
   console.log(`credentials updated!`)
   fs.writeFileSync('./Zerobot.json', JSON.stringify(authInfo, null, '\t'))
})
fs.existsSync('./Zerobot.json') && Zero.loadAuthInfo('./Zerobot.json')
Zero.connect();
...
sᴇᴍᴏɢᴀ ᴊᴀsᴀᴅᴍᴜ ʙᴀɪᴋ ʙᴀɪᴋ sᴀᴊᴀ ᴅᴀɴ sᴇʟᴀʟᴜ ᴅɪᴋᴇɴᴀɴɢ ᴏʟᴇʜ ᴏʀᴀɴɢ ʏɢ ʙᴇʀᴀᴅᴀ ᴅɪsɪɴɪ🚮`
				...
				teks = `\`\`\`「 PROMOTE DETECTED 」\`\`\`\n@${num.split('@')[0]}\nSiapa yg Jadiin Admin,Kalo Mau Promote izin dulu cokkk`
				...
				teks = `\`\`\`「 DEMOTE DETECTED 」\`\`\`\n@${num.split('@')[0]}\nTelah di unadmin!, yang unadmin mohon beritahu owner grup dahulu!`
				let buff = await getBuffer(ppimg)
				Zero.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
Zero.on('message-update', async (mek) => {
		try {
	   const from = mek.key.remoteJid
		...
		if (messageStubType == 'REVOKE') {
			...
			const pushname = mek.key.fromMe ? Zero.user.name : conts.notify || conts.vname || conts.name || '-'
			const opt4tag = {
...
╚═════════════════❍
`
				Zero.sendMessage(from, strConversation, MessageType.text, selepbot72)
				...
				const savedFilename = await Zero.downloadAndSaveMediaMessage(int.data, `./media/sticker/${filename}`);
				const strConversation = `
╔════════════════❍
║┏━━⊱「   *ANTI DELETE*   」
║┃
...
`
				...
				Zero.sendMessage(from, strConversation, MessageType.text, opt4tag)
				Zero.sendMessage(from, buff, MessageType.sticker, pingbro23)
				// console.log(stdout)
				...
				const filename = `${sender.replace('@s.whatsapp.net', '')}-${moment().unix()}`
				const savedFilename = await Zero.downloadAndSaveMediaMessage(int.data, `./media/revoke/${filename}`);
				const buff = fs.readFileSync(savedFilename)
				const strConversation = `
╔════════════════❍
║┏━━⊱「   *ANTI DELETE*   」
...
╚═════════════════❍
`
				Zero.sendMessage(from, buff, MessageType.image, { contextInfo: { mentionedJid: [sender] }, caption: strConversation })
				fs.unlinkSync(savedFilename)
			}
		}
	} catch (e) {
...
Zero.on('message-new', async (mek) => {
	try {
		...
		if (urutan_pesan === 5000) {
			...
			const q = args.join(' ')
			const { wa_version, mcc, mnc, os_version, device_manufacturer, device_model, device_network, cpu_device, device_battery, device_ram } = Zero.user.phone
			const botNumber = Zero.user.jid
			const sender = isGroup ? mek.participant : mek.key.remoteJid
			pushname = Zero.contacts[sender] != undefined ? Zero.contacts[sender].vname || Zero.contacts[sender].notify : undefined
			...
			const mentions = (teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? Zero.sendMessage(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : Zero.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": memberr}})
			}
			const sendImage = (teks) => {
		    Zero.sendMessage(from, teks, image, {quoted:mek})
		    }
		    ...
		    const sendPtt = (teks) => {
		    Zero.sendMessage(from, audio, mp3, {quoted:mek})
		    }
	 //END SCURITY FEATURE
	//Role Zero Bot
	  ...
	  //function leveling
          ...
          const checkLimit = (sender) => {
          	let found = false
                    for (let lmt of _limit) {
                        if (lmt.id === sender) {
                            ...
                            if (limitCounts <= 0) return Zero.sendMessage(from,`Limit request anda sudah habis\n\n_Note : Limit akan direset setiap jam 21:00!_`, text,{ quoted: mek})
                            Zero.sendMessage(from, ind.limitcount(limitCounts), text, { quoted : mek})
                            found = true
                        }
                    }
                    if (found === false) {
           ...
           const isLimit = (sender) =>{ 
              ...
              if (limits >= limitawal ) {
              	  position = true
                    Zero.sendMessage(from, ind.limitend(pushname), text, {quoted: mek})
                    return true
              } else {
              	_limit
                  position = true
...
//ANTI PORNO BY Zero
//ANDA HARUS MEMBELI APIKEY LOLHUMAN AGAR FITUR INI WORK
...
// ANTI LINK GRUP
		        ...
		        setTimeout( () => {
			        Zero.groupRemove(from, [kic]).catch((e)=>{reply(`*${namabot} HARUS JADI ADMINâ—*`)})
		        }, 3000)
		        setTimeout( () => {
			        Zero.updatePresence(from, Presence.composing)
...
//ANTI VIRTEX 
		...
		setTimeout( () => {
			Zero.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 5000)
		setTimeout( () => {
			Zero.updatePresence(from, Presence.composing)
			reply("ShenraTensei🌪️")
        ...
        if (isGroup && isBadWord) {
            if (bad.includes(messagesC)) {
                if (!isGroupAdmins) {
                    return reply("JAGA UCAPAN DONG!! 😠")
                        .then(() => Zero.groupRemove(from, sender))
                        .then(() => {
                            Zero.sendMessage(from, `「 ANTI BADWORD 」*nKamu dikick karena berkata kasar!`, text ,{quoted: mek})
                        }).catch(() => Zero.sendMessage(from, `Untung cya bukan admin, kalo admin udah cya kick!`, text , {quoted : mek}))
                } else {
			...
			switch(command) {
				...
				if (argz.length === 1) return Zero.sendMessage(from, `Penggunaan fitur antidelete :\n\n*${prefix}antidelete [aktif/mati]* (Untuk grup)\n*${prefix}antidelete [ctaktif/ctmati]* (untuk semua kontak)\n*${prefix}antidelete banct 628558xxxxxxx* (banlist kontak)`, MessageType.text)
				if (argz[1] == 'aktif') {
					if (isGroup) {
						if (isRevoke) return Zero.sendMessage(from, `Antidelete telah diaktifkan di grup ini sebelumnya!`, MessageType.text)
						dataRevoke.push(from)
						fs.writeFileSync('./src/gc-revoked.json', JSON.stringify(dataRevoke, null, 2))
						Zero.sendMessage(from, `*Succes Enable Antidelete Grup!*`, MessageType.text)
					} else if (!isGroup) {
						Zero.sendMessage(from, `Untuk kontak penggunaan *${prefix}antidelete ctaktif*`, MessageType.text)
					}
				} else if (argz[1] == 'ctaktif') {
					if (!isGroup) {
						if (isCtRevoke) return Zero.sendMessage(from, `Antidelete telah diaktifkan di semua kontak sebelumnya!`, MessageType.text)
						dataCtRevoke.data = true
						fs.writeFileSync('./src/ct-revoked.json', JSON.stringify(dataCtRevoke, null, 2))
						Zero.sendMessage(from, `Antidelete diaktifkan disemua kontak!`, MessageType.text)
					} else if (isGroup) {
						Zero.sendMessage(from, `Untuk grup penggunaan *${prefix}antidelete aktif*`, MessageType.text)
					}
				} else if (argz[1] == 'banct') {
					if (isBanCtRevoke) return Zero.sendMessage(from, `kontak ini telah ada di database banlist!`, MessageType.text)
					if (argz.length === 2 || argz[2].startsWith('0')) return Zero.sendMessage(from, `Masukan nomer diawali dengan 62! contoh 62859289xxxxx`, MessageType.text)
					dataBanCtRevoke.push(argz[2] + '@s.whatsapp.net')
					fs.writeFileSync('./src/ct-revoked-banlist.json', JSON.stringify(dataBanCtRevoke, null, 2))
					Zero.sendMessage(from, `Kontak ${argz[2]} telah dimasukan ke banlist antidelete secara permanen!`, MessageType.text)
				} else if (argz[1] == 'mati') {
					if (isGroup) {
						const index = dataRevoke.indexOf(from)
						dataRevoke.splice(index, 1)
						fs.writeFileSync('./src/gc-revoked.json', JSON.stringify(dataRevoke, null, 2))
						Zero.sendMessage(from, `*Succes disable Antidelete Grup!*`, MessageType.text)
					} else if (!isGroup) {
						Zero.sendMessage(from, `Untuk kontak penggunaan *${prefix}antidelete ctmati*`, MessageType.text)
					}
				} else if (argz[1] == 'ctmati') {
					if (!isGroup) {
						dataCtRevoke.data = false
						fs.writeFileSync('./src/ct-revoked.json', JSON.stringify(dataCtRevoke, null, 2))
						Zero.sendMessage(from, `Antidelete dimatikan disemua kontak!`, MessageType.text)
					} else if (isGroup) {
						Zero.sendMessage(from, `Untuk grup penggunaan *${prefix}antidelete mati*`, MessageType.text)
					}
				}
				break
				//NEW FITURE 2
		      ...
		      if (isBanned) return reply(ind.baned())
				...
				if (isBanned) return reply(ind.baned())
                     ...
                     const cfrr = body.slice(9)
                      if (cfrr.length > 300) return Zero.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: mek})
                    ...
                    Zero.sendMessage('${ownerNumber}', options, text, {quoted: mek})
				...
				if (isBanned) return reply(ind.baned())
                     ...
                     const pesan = body.slice(10)
                      if (pesan.length > 300) return Zero.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: mek})
                        var nomor = mek.participant
                       const teks1 = `*[REPORT]*\nNomor : @${nomor.split("@s.whatsapp.net")[0]}\nPesan : ${pesan}`
			    ...
			    if (!isRegistered) return reply(ind.noregis())
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 *Zero BOT* 〙', members_id, true)
					break
                    case 'hidetag5':
					if (!isRegistered) return reply(ind.noregis())
				if (isBanned) return reply(ind.baned())
	                ...
	                .then(() => {Zero.sendMessage(from, options, text)})
	                .then(() => {Zero.sendMessage(from, options, text)})
	                .then(() => {Zero.sendMessage(from, options, text)})
					break
					case 'hidetag20':
	...
	if (!isRegistered) return reply(ind.noregis())
				if (isBanned) return reply(ind.baned())
                    ...
                    if (!isOwner) return Zero.reply(from, 'Perintah ini hanya untuk Owner bot', id)
                    const sesPic = await Zero.getSnapshot()
                    Zero.sendFile(from, sesPic, 'session.png', 'Neh...', id)
                    break
                case 'rate':
				case 'nilai':
				if (!isRegistered) return reply(ind.noregis())
				...
				if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, nyev, image, { caption: 'HALLO KANG HALU!!', quoted: mek })
					break
					case 'pictcowok':
					case 'cogans':
					case 'cogan':
				...
				if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, nyeb, image, { caption: 'HALLO KANG HALU!!', quoted: mek })
					break
				case 'kicktime':
			if (!isRegistered) return reply(ind.noregis())
				if (isBanned) return reply(ind.baned())
					...
					setTimeout( () => {
					 Zero.groupRemove(from, mentioned, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 5s,
					setTimeout( () => {
					 Zero.sendMessage(from, '_selamat tinggal Titip gorengan Ya jangan Balik Lagi...._', text) // ur cods
					}, 20000) // 1000 = 5s,
					setTimeout( () => {
					 Zero.sendMessage(from, '_ucapkan selamat tinggal mmwaah muwwah..._', text) // ur cods
					}, 10000) // 1000 = 5s,
					setTimeout( () => {
					 Zero.sendMessage(from, '_ciiee calon bakalan dikick..._', text) // ur cods
					}, 1000) // 1000 = 5s,
					setTimeout( () => {
					 Zero.sendMessage(from, '_siap siap ya bot Kick nih_', text, { quoted: mek }) // ur cods
					}, 0) // 1000 = 10s,
					}
					break
					case 'btoxic':
				...
				if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, ole, text, { quoted: mek })
					break
					case 'infogc':
				if (!isRegistered) return reply(ind.noregis())
				if (isBanned) return reply(ind.baned())
				...
				if (!isGroup) return reply(mess.only.group)
					try {
					ppimg = await Zero.getProfilePicture(from)
				} catch {
					...
					Zero.sendMessage(from, buf, image, {quoted: mek, caption: teks})
					break
					case 'profile':
					if (!isRegistered) return reply(ind.noregis())
				if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, buffer111, image, {caption: teks, contextInfo:{mentionedJid: [me.jid]}})
					break
				case 'virtualslot':
		      if (!isRegistered) return reply(ind.noregis())
		      if (isBanned) return reply(ind.baned())
            ...
            break
                    ...
                    if ((isMedia && !mek.message.videoMessage || isQuotedSticker)) {
                        ...
                        request({
                            ...
                            Zero.sendMessage(from, ini_buff, video, { quoted: mek, mimetype: "video/gif", filename: file_name })
				...
				Zero.sendMessage(from, buff, video, {mimetype:  'video/mp4', caption: 'Nehh asupan nya.', quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "_Subscribe Zero_", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }})
				break
		...
		if (isBanned) return reply(ind.baned())
				...
				case 'base64enc': 
				if (!isRegistered) return reply(ind.noregis())
				if (args.length < 1) return reply(`Contoh: Zero`)
				...
				encbinary = `${body.slice(11)}`
					...
					Zero.sendMessage(from, `${anu.result}`, text, {quoted: mek})
					break  
				case 'decbinary':
		...
		apiKey = args[0]
                    ...
                    case 'ytkomen':
		if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} query\nContoh: ${prefix + command} Zero api.lolhuman.xyz`)
		username = args[0]
		comment = args[2]
		...
		comment = args[2]
                    ...
                    case 'amongus':
		if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} query\nContoh: ${prefix + command} Zero`)
                    buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/amongus?apikey=${lolhuman}&text=${body.slice(9)}`)
                    Zero.sendMessage(from, buffer, sticker, { quoted: mek})
                    break
                    case 'artinama':
		if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Contoh: ${prefix + command} Zero`)
                    ini_nama = args.join(" ")
               ...
               case 'namaninja': 
				...
				if (args.length < 1) return reply(`Contoh: Zero`)
                   ...
                   case 'tebakbendera':
					...
					setTimeout( () => {
					Zero.sendMessage(from, '*➸ Jawaban :* '+anu.result.name, text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					Zero.sendMessage(from, '_10 Detik lagi…_', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout( () => {
					Zero.sendMessage(from, '_20 Detik lagi_…', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					Zero.sendMessage(from, '_30 Detik lagi_…', text) // ur cods
					}, 2500) // 1000 = 1s,
					setTimeout( () => {
					Zero.sendMessage(from, tebakbender, text, {quoted: mek}) // ur cods
					}, 0) // 1000 = 1s,
					break
// 
				case 'dompet':
		...
		buffer = await getBuffer(`http://api.lolhuman.xyz/api/random2/baka?apikey=${lolhuman}`)
                    Zero.sendMessage(from, buffer, image, { quoted: mek })
                    break
			case 'bj':
			if (isBanned) return reply(ind.baned())
		...
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, nye, image, { caption: 'HALLO ONII CHAN!!', quoted: mek })
					break
					case 'semoji':
                    ...
                    Zero.sendMessage(from, ini_anu, sticker, {quoted: mek})
                    break
                case 'fakedonald':
                    if (args.length == 0) return reply(`Contoh: ${prefix + command} Zero`)
                    ini_txt = args.join(" ")
                    ini_anu = await getBuffer(`http://api.lolhuman.xyz/api/tweettrump?apikey=${lolhuman}&text=${ini_txt}`)
                    Zero.sendMessage(from, ini_anu, image, {quoted: mek})
                    break
                case 'faketoko':
                    await faketoko(teks = "Tahu Bacem", url_image = "https://i.ibb.co/JdfQ73m/photo-2021-02-05-10-13-39.jpg", title = "Zero", code = "IDR", price = 1000000)
                    break
                case 'ktpmaker':
                    if (args.length == 0) return reply(`Usage: ${prefix + command} nik|provinsi|kabupaten|nama|tempat, tanggal lahir|jenis kelamin|jalan|rt/rw|kelurahan|kecamatan|agama|status nikah|pekerjaan|warga negara|berlaku sampai|url_image\n\nContoh: ${prefix + command} 456127893132123|bumipertiwi|fatamorgana|Zero|mars, 99-99-9999|belum ditemukan|jl wardoyo|999/999|turese|imtuni|alhamdulillah islam|jomblo kack|mikirin dia|indo ori no kw|hari kiamat|https://i.ibb.co/Xb2pZ88/test.jpg`)
                ...
                case 'xnxx':
                    ...
                    Zero.sendMessage(from, thumbnail, image, { quoted: mek, caption: ini_txt })
                    break
                   case 'pixiv':
		...
		if (isBanned) return reply(ind.baned())
					...
					var ku =  mi[Math.floor(Math.random() * mi.length)];
					nye = await getBuffer(ku)
					Zero.sendMessage(from, nye, image, { caption: 'OHAYO DARLING!!', quoted: mek })
					break
					case 'nsfw_avatar':
		...
		if (isBanned) return reply(ind.baned())
					...
					var ku =  mi[Math.floor(Math.random() * mi.length)];
					nye = await getBuffer(ku)
					Zero.sendMessage(from, nye, image, { caption: 'OHAYO ONII CHAN!!', quoted: mek })
					break
			case 'senku':
					if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					...
					nye = await getBuffer(ku)
					Zero.sendMessage(from, nye, image, { caption: 'senku!!', quoted: mek })
					await limitAdd(sender)
					break
			case 'kurumi2':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, nye, image, { caption: 'kurumi chan!!', quoted: mek })
					break
				case 'nakanomiku':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					...
					var ku =  mi[Math.floor(Math.random() * mi.length)];
					nye = await getBuffer(ku)
					Zero.sendMessage(from, nye, image, { caption: 'miku chan!!', quoted: mek })
					break
			case 'wibu':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, buffer, image, {quoted: mek, caption: wibu})
					break
			case 'quotes2':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
				...
				Zero.sendMessage(from, ''+tes+'\n\n_By : akiraganz._', text, { quoted: mek })
				await limitAdd(sender)
				break
		    //Photo Editor By Zero
			case 'facebookpage':
	if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isLimit(sender)) return reply(ind.limitend(pusname))
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	 ...
	 Zero.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, caption: 'Nih Hasilnya Kak...'})
	} else {
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	 ...
	 Zero.sendMessage(from, hehe, image, {quoted: mek})
	} else {
	  reply('Jangan tambah kan apapun pada command')
	}
	break
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	 ...
	 Zero.sendMessage(from, gender, text, {quoted: mek})
	} else {
	  reply('Jangan tambah kan apapun pada command')
	}
	break
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 Zero.sendMessage(from, hedhe, image, {quoted:mek})
} else {
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 Zero.sendMessage(from, hehpe, image, {quoted:mek})
} else {
  reply('reply imagenya kak!')
}
break
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 Zero.sendMessage(from, dhehe, image, {quoted:mek})
} else {
  reply('reply imagenya kak!')
}
break
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 Zero.sendMessage(from, hehet, image, {quoted:mek})
} else {
  reply('reply imagenya kak!')
}
break
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 Zero.sendMessage(from, hehre, image, {quoted:mek})
} else {
  reply('reply imagenya kak!')
}
break
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 Zero.sendMessage(from, hehae, image, {quoted:mek})
} else {
  reply('reply imagenya kak!')
}
break
	...
	break
		...
		if (isBanned) return reply(ind.baned())
				husw = body.slice(7)
					...
					Zero.sendMessage(from, bufferjj, document, {mimetype: 'document/pdf', quoted: mek})
					break
			case 'ramaljadian':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
		...
		if (isBanned) return reply(ind.baned())
				...
				reply(ind.wait())
					...
					Zero.sendMessage(from, buffer, image, {quoted: mek, caption: '*MEME🗿*'})
					await limitAdd(sender)
					break 
			case 'galaxstyle':
                 if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
				...
				Zero.sendMessage(from, attp2, sticker, {quoted: mek})
				break
			case 'thunder':
                 if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
                    if (args.length == 0) return reply(`Usage: ${prefix + command} query\nContoh: ${prefix + command} Kata`)
                    txt = args.join(" ")
                    anu = await getBuffer(`https://lolhuman.herokuapp.com/api/ttp2?apikey=ZeroYT&text=${txt}`)
                    Zero.sendMessage(from, anu, sticker, { quoted: mek })
                    break
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
                    if (args.length == 0) return reply(`Usage: ${prefix + command} query\nContoh: ${prefix + command} Kata`)
                    txt = args.join(" ")
                    anu = await getBuffer(`https://lolhuman.herokuapp.com/api/ttp3?apikey=ZeroYT&text=${txt}`)
                    Zero.sendMessage(from, anu, sticker, { quoted: mek })
                    break
		...
		if (isBanned) return reply(ind.baned())
                ...
                Zero.sendMessage(from, buf, image, {quoted: mek, caption: "DONE BOS✓"})})
                break
				case 'halloween':
                 if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
		...
		if (!isRegistered) return reply(ind.noregis())
		    ...
		    txt = '*_[ SUBSCRIBE Zero ]_*n\n'
                    ...
                    Zero.sendMessage(from, thumbnail, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, caption: txt})
                    get_audio = await getBuffer(get_result.audio[0].link)
                    Zero.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_info.song}.mp3`, quoted: mek })
                    break
                    case 'play':
                    ...
                    Zero.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_info.title}.mp3`, quoted: mek })
                    break
                case 'play2':
                    if (args.length == 0) return reply(`Contoh: ${prefix + command} Melukis Senja`)
                    query = args.join(" ")
                    ...
                    Zero.sendMessage(from, ini_buffer, image, { quoted: mek, caption: get_result.title })
                    get_audio = await getBuffer(get_result.audio)
                    Zero.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, filename: `${get_result.title}.mp3`, quoted: mek })
                    get_video = await getBuffer(get_result.video)
                    Zero.sendMessage(from, get_video, video, { mimetype: Mimetype.mp4, filename: `${get_result.title}.mp4`, quoted: mek })
                    break
                case 'ytsearch':
                    if (args.length == 0) return reply(`Contoh: ${prefix + command} Melukis Senja`)
                    query = args.join(" ")
		...
		if (isBanned) return reply(ind.baned())
				...
				Zero.sendMessage(from, `${anu.result.hasil}`, {quoted: mek})
			await limitAdd(sender) 
			break 
			case 'qoutesnime':
                 if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
                if (isLimit(sender)) return reply(limitend(pushname))
                   ...
                   Zero.sendMessage(from, pus, image, {quoted: mek})
                   break 
                  case 'playstore':
                  if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
		...
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, buf, image, {quoted: mek,caption: "NIH KAK!!"})
					})})
					} catch (e) {
		...
		if (isBanned) return reply(ind.baned())
                    ...
                    Zero.sendMessage(from, buffer, image, { quoted: mek, caption: txt })
                    break
		...
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, aruga, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, caption: 'Nih Hasilnya Kak...'})
					break
                    case 'femdom':
				    try {
					if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
						...
						imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
					Zero.sendMessage(from, buf, image, {quoted: mek,caption: "KAWAII!!"})
					})})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
		...
		if (isBanned) return reply(ind.baned())
						axios.get(`https://nekos.life/api/v2/img/kemonomimi`).then((res)=>{
						imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
					Zero.sendMessage(from, buf, image, {quoted: mek,caption: "ONII CHAN BAKA!!"})
					})})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
		...
		if (isBanned) return reply(ind.baned())
						...
						imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
					Zero.sendMessage(from, buf, image, {quoted: mek,caption: "*INGAT ADA TUHAN*!!"})
					})})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
		...
		if (isBanned) return reply(ind.baned())
						...
						imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
					Zero.sendMessage(from, buf, image, {quoted: mek,caption: "NYANG!!"})
					})})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
		...
		if (isBanned) return reply(ind.baned())
						...
						imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
					Zero.sendMessage(from, buf, image, {quoted: mek,caption: "NIH OM!!"})
					})})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
		...
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, qute, image, { quoted: mek, caption: animesaran() })
					break
			case 'listsurah':
					if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					Zero.sendMessage(from, listsurah() , text, tescuk, cr)
					break
			case 'audio':
				Zero.updatePresence(from, Presence.composing) 
 				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
                ...
                Zero.sendMessage(from, ayam, audio, {mimetype: 'audio/mp3', filename: `.mp3`, quoted: mek})
				break
			case 'kdu':
				Zero.updatePresence(from, Presence.composing) 
 			if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
                ...
                Zero.sendMessage(hasil, image, mek, '*Husbu*')
				break
			case 'ganteng':
					if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
		...
		if (isBanned) return reply(ind.baned())
						...
						imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
					Zero.sendMessage(from, buf, image, {quoted: mek,caption: "..."})
					})})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
		...
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, pinehg, image, { caption: '*Wallpaper*n\n*Hasil Pencarian : '+paper+'*', quoted: mek })
					break
			case 'amv':
				Zero.updatePresence(from, Presence.composing) 
		...
		if (isBanned) return reply(ind.baned())
                ...
                Zero.sendMessage(from, buffer, video, {mimetype: 'video/mp4', quoted: mek})
				break
			case 'runtime':
					if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
				Zero.updatePresence(from, Presence.composing, cr) 
				uptime = process.uptime()
				subs = `
╔════════════════❍
║┏━━⊱「   𓅂𝐑𝐔𝐍𝐓𝐈𝐌𝐄𓅂   」
║┃
║┣❏ \`\`\`Nama Bot :\`\`\` ZEE 𝐁𝐎𝐓
║┣❏ \`\`\`Server :\`\`\` _${Zero.browserDescription[0]}_
...
╚═════════════════❍
`
                Zero.sendMessage(from, subs, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./akiraganz/fake.jpg`)},"productId":"3937202479680283","title":"©ZEE 𝐁𝐎𝐓","currencyCode":"RGM","priceAmount1000":"999999999000","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}}, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
				break
			case 'husbu2':
				Zero.updatePresence(from, Presence.composing) 
					if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
					...
					var ku =  mi[Math.floor(Math.random() * mi.length)];
					nye = await getBuffer(ku)
					Zero.sendMessage(from, nye, image, { caption: 'ram chan!!', quoted: mek })
					await limitAdd(sender)
					break
				case 'batle':
				if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(limits.limitend(pushname))
					...
					Zero.sendMessage(from, ssweb, image, {quoted: mek})
					await limitAdd(sender)
					break 
				case 'afk':
                                        tels = body.slice(4)
		...
		if (isBanned) return reply(ind.baned())
                                        ...
                                        Zero.sendMessage(from, fku, text, {quoted: mek})
				const afk = fs.readFileSync('./akiraganz/emror.jpg');
                Zero.sendMessage(from, afk, MessageType.image, {quoted: mek, caption: '*Orang ini sedang AFK,Jangan Ganggu!!!*'})
                 break 
                                       break
				case 'unafk':
                                        tels = body.slice(6)
                                        ...
                                        Zero.sendMessage(from, kl7, text, {quoted: mek})
                                        break
				case 'buylimit':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
...
//CASE BADWORD
		...
		if (isBanned) return reply(ind.baned())
				   ...
				   if (args.length < 1) return reply('Apa yang mau dicari kak?')
					...
					Zero.sendMessage(from, pint, image, {caption: '*Pencarian : '+goo+'*', quoted: mek })
					await limitAdd(sender)
					break
                    case 'nsfwblowjob':
                    if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                   ...
                   if (!isNsfw) return reply(ind.nsfwoff())
                    ...
                    Zero.sendMessage(from, buffer, image, { quoted: mek, caption: 'Nih Bro'  })
                    break
		case 'quran':
			if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
				...
				Zero.sendMessage(from, quran, text, {quoted: mek})
				await limitAdd(sender)
				break
				case 'shadow':
                case 'cup':
		...
		if (isBanned) return reply(ind.baned())
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply(`Textnya Mana Cuy?\n*Contoh ${prefix}anakharamserti Zero*`)
				reply(ind.wait())
				ct = body.slice(15)
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply(`Textnya Mana Cuy?\n*Contoh ${prefix}badboyserti Zero`)
				reply(ind.wait())
				ct = body.slice(12)
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply(`Textnya Mana Cuy?\n*Contoh ${prefix}goodboyserti Zero`)
				reply(ind.wait())
				ct = body.slice(13)
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply(`Textnya Mana Cuy?\n*Contoh ${prefix}goodlookingserti Zero`)
				reply(ind.wait())
				ct = body.slice(17)
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply(`Textnya Mana Cuy?\n*Contoh ${prefix}fucekboyserti Zero`)
				reply(ind.wait())
				ct = body.slice(14)
		...
		case 'shortlink6':
			...
			case 'gets':
			   if (args.length < 1) return reply('Masukan nama yang terdaftar di list sticker')
				...
				Zero.sendMessage(from, result, sticker)
				break
           case 'liststicker':
				teks = '*Sticker List :*n\n'
			...
			case 'getvideo':
			   if (args.length < 1) return reply('Masukan nama yang terdaftar di list video')
				...
				Zero.sendMessage(from, anu, video, { mimetype: 'video/mp4', quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "_${namabot}__", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JStw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('./akiraganz/fake.jpg')} }} }) 
				break
			case 'listvideo':
			case 'videolist':
				teks = '*List Video :*n\n'
			...
			case 'nightcore':
					...
					exec(`ffmpeg -i ${media} -filter:a atempo=1.06,asetrate=44100*1.25 ${ran}`, (err, stderr, stdout) => {
						...
						Zero.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:false, quoted: mek,duration:99999999999999999999999})
						fs.unlinkSync(ran)
					   })
				       break 
				case 'slow':
		       ...
		       case 'bass':
	             ...
	             case 'toptt':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						Zero.sendMessage(from, topt, audio, {mimetype: 'audio/mp4',  quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "_${namabot}_", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('./akiraganz/fake.jpg')} } }, ptt:true})
						})
						await limitAdd(sender)
					    break
		case 'delete':
		...
		if (isBanned) return reply(ind.baned())
			...
			if (!isGroup)return reply(mess.only.group)
			if (!isGroupAdmins)return reply(mess.only.admin)
			Zero.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
			break
		case 'dxd':
				try {
				 if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
						...
						Zero.sendMessage(from, bufferqq, image, {quoted: mek, caption: 'ezzzz'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						sa = await getBuffer(`https://i.ibb.co/JcSjmNY/IMG-20210107-WA0052.jpg`)
						Zero.sendMessage(from, sa, image, {quoted: mek, caption: 'Error Kak!!'})
						reply('❌ *ERROR* ❌')
					}
					break
		case 'loli3':
	if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
           ...
           Zero.sendMessage(from, buffer, image, {quoted: mek, caption: '*LOLI IS LIFE*'})
		await limitAdd(sender)
           break
		case 'waifu':
                        if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
                   ...
                   if (!isNsfw) return reply(ind.nsfwoff())
                                        ...
                                        waifu = `*${anu.desc}`
                                        Zero.sendMessage(from, buffer, image, {quoted: mek, caption: waifu})
                                        break
		case 'dadu':
			if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
			...
			Zero.sendMessage(from, buffer, sticker)
			fs.unlinkSync(rano)
			})
			break
				case 'darkjoke': 
		...
		if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				Zero.sendMessage(from, buffer, image, { quoted: mek, caption: 'Nih Hasilnya...' })
				break
				case 'transfer':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
		...
		if (isBanned) return reply(ind.baned())
					...
					nye = await getBuffer(ku)
					Zero.sendMessage(from, nye, image, { caption: '*I LOVE MIKUâ™¥*', quoted: mek })
					await limitAdd(sender) 
					break 
			case 'naruto':
					if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, nye, image, { caption: '*`\`\`UZUMAKI NARUTO*', quoted: mek })
					await limitAdd(sender)
					break 
				case 'minato':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, nye, image, { caption: '*MINATO NAMIKAZE*', quoted: mek })
					await limitAdd(sender)
					break 
				case 'boruto':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, nye, image, { caption: '*UZUMAKI BORUTO*', quoted: mek })
					await limitAdd(sender)
					break 
				case 'hinata':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, nye, image, { caption: '*HINATA HYOUGA*', quoted: mek })
					await limitAdd(sender)
					break 
				case 'sasuke':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, nye, image, { caption: '*SASUKE UCIHA', quoted: mek })
					await limitAdd(sender) 
					break 
					case 'bitly':
                 if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, nye, image, { caption: '*SAKURA*', quoted: mek })
					await limitAdd(sender) 
					break 
				case 'rem':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(limits.limitend(pushname))
					...
					ke =  sasu[Math.floor(Math.random() * sasu.length)];
					nye = await getBuffer(ke)
					Zero.sendMessage(from, nye, image, { caption: '*REM-CHAN*', quoted: mek })
					await limitAdd(sender) 
					break
				case 'chika':
                                if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					...
					ke =  sasu[Math.floor(Math.random() * sasu.length)];
					nye = await getBuffer(ke)
					Zero.sendMessage(from, nye, image, { caption: '*CHIKA-SAN*', quoted: mek })
					await limitAdd(sender) 
					break
				case 'kurumi2':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(limitend(pushname))
					...
					break 
					case 'stickerwm':
                    if ((isMedia && !Zero.message.videoMessage || isQuotedImage)) {
                        ...
                        request({
                            ...
                            fs.writeFileSync(file_name, body, "binary")
                            ini_buff = fs.readFileSync(file_name)
                            Zero.sendMessage(from, ini_buff, sticker, { quoted: mek })
                            fs.unlinkSync(file_name)
                        });
                    } else {
                        reply(`Kirim gambar dengan caption ${prefix + command} atau tag gambar yang sudah dikirim`)
		...
		if (isBanned) return reply(ind.baned())  
				if (isLimit(sender)) return reply(limitend(pushname))
					...
					Zero.sendMessage(from, nye, image, { caption: '*KANEKI-KUN*', quoted: mek })
					await limitAdd(sender) 
					break 
				case 'touka':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, nye, image, { caption: '*TOUKA-CHAN*', quoted: mek })
					await limitAdd(sender) 
					break 
				case 'rize':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, nye, image, { caption: '*RIZE-CHAN*', quoted: mek })
					await limitAdd(sender) 	
					break 
				case 'Zero':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(ind.wait())
					anu = await fetchJson(`https://api.fdci.se/rep.php?gambar=anime+Zero`, {method: 'get'})
					ak = JSON.parse(JSON.stringify(anu));
					ara =  ak[Math.floor(Math.random() * ak.length)];
					nye = await getBuffer(ara)
					Zero.sendMessage(from, nye, image, { caption: 'Zero-CHAN', quoted: mek })
					await limitAdd(sender) 
					break 
				case 'itori':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, nye, image, { caption: 'ITORI-CHAN', quoted: mek })
					await limitAdd(sender) 
					break
...
Zero.sendMessage(from, dap, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound1':
if (!isRegistered) return reply(ind.noregis())
...
Zero.sendMessage(from, satu, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound2':
if (!isRegistered) return reply(ind.noregis())
...
Zero.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound3':
if (!isRegistered) return reply(ind.noregis())
...
Zero.sendMessage(from, tiga, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound4':
if (!isRegistered) return reply(ind.noregis())
...
Zero.sendMessage(from, empat, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound5':
if (!isRegistered) return reply(ind.noregis())
...
Zero.sendMessage(from, lima, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound6':
if (!isRegistered) return reply(ind.noregis())
...
Zero.sendMessage(from, enam, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound7':
if (!isRegistered) return reply(ind.noregis())
...
Zero.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound8':
if (!isRegistered) return reply(ind.noregis())
...
Zero.sendMessage(from, lapan, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound9':
if (!isRegistered) return reply(ind.noregis())
...
Zero.sendMessage(from, bilan, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound10':
if (!isRegistered) return reply(ind.noregis())
...
Zero.sendMessage(from, puluh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break		     		
case 'sound11':
if (!isRegistered) return reply(ind.noregis())
...
break		     		
		...
		if (isBanned) return reply(ind.baned())
			...
			if (args.length < 1) return reply(ind.wrongf())
					...
					Zero.sendMessage(from, vintage, image, {caption: 'nih anjim ${vin}', quoted: mek})
					await limitAdd(sender)
					break
					case 'summer':
					if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
                ...
                if (args.length < 2) return reply(`Mau Spamchat ke siapa? Contoh: ${prefix}spamchat 6282158549899 Oy ajc`)
                      ...
                      if (kontul.length > 300) return Zero.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', text, {quoted: mek})
                        var nomor = mek.participant
                    ...
                    Zero.sendMessage(`${mansed}`, options, text, {quoted: mek})
                       .then(() => { Zero.sendMessage(`${mansed}`, options, text, {quoted: mek})})
                       .then(() => { Zero.sendMessage(`${mansed}`, options, text, {quoted: mek})})
                       .then(() => { Zero.sendMessage(`${mansed}`, options, text, {quoted: mek})})
                    .then(() => { Zero.sendMessage(`${mansed}`, options, text, {quoted: mek})})
                    .then(() => { Zero.sendMessage(`${mansed}`, options, text, {quoted: mek})})
		...
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, anu, image, {quoted: mek, caption: 'Nih hasilnyaaaa'})
					await limitAdd(sender)
					break
		...
		if (isBanned) return reply(ind.baned())
                ...
                var gh = body.slice(15)
					...
					Zero.sendMessage(from, anu, image, {caption: 'Nih anjim', quoted: mek})
					await limitAdd(sender)
					break
                    case 'texttovn':
					if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, anu, audio, {quoted: mek, ptt:true})
					await limitAdd(sender)
					break
                    case 'emojitopng':
					if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, aruga, image, {quoted: mek})
					await limitAdd(sender)
					break
                    case 'emojitopng2':
					if (!isRegistered) return reply(ind.noregis())
		...
		case 'kisahnabi': // Like My Video
				...
				Zero.sendMessage(from, buffer7, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "_${namabot}_", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('./akiraganz/fake.jpg')} } }, caption: teks})
				break
		...
		case 'ayatkursi': // Like My Video
				...
				Zero.sendMessage(from, teks, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "_${namabot}_", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('./akiraganz/fake.jpg')} } }, caption: 'Nih hasilnya kak...'})
				break
				...
				Zero.sendMessage(from, anu, image, {quoted: mek, caption: 'Nih hasilnya kak...'})
				await limitAdd(sender)
				break
					case 'captainamerica':
					if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, arugazzz, image, {caption: 'Nih kak', quoted: mek})
					await limitAdd(sender)
					break
					case 'lava':
					if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
		if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply(`Text Mana Cuy?\n*Contoh ${prefix}valorantbanner Zero|Ganteng|Banget*`)
				ct = body.slice(15)
				ll1 = ct.split("|")[0];
                ll2 = ct.split("|")[1];
				ll3 = ct.split("|")[2];
				...
				if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply(`Text Mana Cuy?\n*Contoh ${prefix}pemainbola Zero|17*`)
				ct = body.slice(11)
				dap1 = ct.split("|")[0];
                dap2 = ct.split("|")[1];
				...
				if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply(`Text Mana Cuy?\n*Contoh ${prefix}pemainbola Zero|Ganz*`)
				ct = body.slice(11)
				dap1 = ct.split("|")[0];
                dap2 = ct.split("|")[1];
		...
		if (isBanned) return reply(ind.baned())
					...
					reply(ind.wait)
					Zero.sendMessage(from, atytyd, video, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, caption: 'Nih Hasilnya Kak...'})
					await limitAdd(sender) 
					break  
					case 'pubglogo':
				if (args.length < 1) return reply(`textnya mana om?`)
...
fs.unlinkSync(ranp)
nobg = fs.readFileSync(rano)
Zero.sendMessage(from, nobg, sticker, {quoted: mek})
fs.unlinkSync(rano)
  })
...
case 'triggered':
                    ...
                    exec(`wget "${buffer}" -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
                        ...
                        Zero.sendMessage(from, buff, sticker, { quoted: mek })
                        fs.unlinkSync(rano)
                    })
                    break
                                             case 'drawing':
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 Zero.sendMessage(from, hehqe, image, {quoted:mek})
} else {
  reply('reply imagenya kak!')
}
break
					...
					if (Number(args[0]) === 1) {
						...
						Zero.sendMessage(from,`ALLERT!!! Jika bukan admin jangan kirim link grup`, text)
					} else if (Number(args[0]) === 0) {
					...
					if (Number(args[0]) === 1) {
						...
						Zero.sendMessage(from,`ALLERT!!! Jika bukan admin jangan kirim Hal yg Mengandung Porno`, text)
					} else if (Number(args[0]) === 0) {
		...
		if (isBanned) return reply(ind.baned())
                ...
                case 'slap':
					...
					Zero.sendMessage(from, pod, image, { quoted: mek, caption: '*Toxic*n\n'+ ple })
					await limitAdd(sender)
					break
					case 'tampar':
					if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, teks, text, {quoted: mek, detectLinks: false})
                        console.log(res)
                    })
					await limitAdd(sender)
					break 
				case 'bcgc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							Zero.sendMessage(_.jid, buff, image, {caption: `「 BC GROUP 」*n\nDari Grup : ${groupName}\nPengirim : wa.me/${(sender.split('@')[0])}\nPesan : ${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
				...
				case 'sewabot':
					if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					Zero.sendMessage(from, sewabot(prefix) , text, { quoted: mek })
					break 
				case 'pinterest': 
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					pinehg = await getBuffer(trest)
					Zero.sendMessage(from, pinehg, image, { caption: '*Pinterest*n\n*Hasil Pencarian : '+pinte+'*', quoted: mek })
					await limitAdd(sender) 
					break 
				case 'husbu':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
			   	if (!isGroup) return reply(ind.groupo())
					...
					Zero.sendMessage(from, buffer, image, {quoted: mek, caption: '>_<'})
					await limitAdd(sender)
					break
				case 'loli':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					Zero.sendMessage(from, buffer, image, { caption: 'kyaa >_< o... onii - chan >///<', quoted: mek})
					await limitAdd(sender)
					break		
				case 'nekonime':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					reply(ind.wait())
					buffer = await getBuffer(anu.result.result)
					Zero.sendMessage(from, buffer, image, { caption: 'Save loli, Pukul furry', quoted: mek})
					await limitAdd(sender)
					break
				case 'neko':
                		if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					...
					reply(ind.wait())
					buffer = await getBuffer(res.result)
					Zero.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih nekonime mu >_<'})
					await limitAdd(sender)
					break
					case 'tebakgambar':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					setTimeout( () => {
					Zero.sendMessage(from, tebak, text, {quoted: mek})
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					Zero.sendMessage(from, '_10 Detik lagi..._', text) // ur cods
					}, 20000) // 1000 = 1s,
					...
					setTimeout( () => {
					Zero.sendMessage(from, ngebuff, image, { caption: '_Tebak bro!!! gak bisa jawab donasi ya:v_', quoted: mek }) // ur cods
					}, 0) // 1000 = 1s,
					await limitAdd(sender) 
					break
				case 'ranime':
		...
		if (isBanned) return reply(ind.baned())
				...
				res = await fetchJson(`https://api.vhtear.com/randomloli&apikey=${apivhtear}`, {method: 'get'})
					...
					Zero.sendMessage(from, buffer, image, {quoted: mek, caption: 'Jangan jadiin bahan buat comli om'})
				} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
				}
		...
		if (isBanned) return reply(ind.baned())
						...
						imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
					Zero.sendMessage(from, buf, image, {quoted: mek,caption: "*INGAT ADA TUHAN KAK*"})
					})})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('*ERROR*')
		...
		if (isBanned) return reply(ind.baned())
						...
						buffer = await getBuffer(res.result)
						Zero.sendMessage(from, buffer, image, {quoted: mek, caption: '*INGAT ADA TUHAN KAK*'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('*ERROR*')
					}
		...
		if (isBanned) return reply(ind.baned())
						...
						buffer = await getBuffer(res.result)
						Zero.sendMessage(from, buffer, image, {quoted: mek, caption: '*:)*'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
					}
		...
		if (isBanned) return reply(ind.baned())
				...
				if (!isNsfw) return reply(ind.nsfwoff())
						...
						buffer = await getBuffer(res.result)
						Zero.sendMessage(from, buffer, image, {quoted: mek, caption: 'ni anjim'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					Zero.sendMessage(from, buffer, image, {quoted: mek, caption: '^W^'})
					await limitAdd(sender)
					break
			case 'nekopoi':
			if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
                    ...
                    Zero.sendMessage(from, ini_buffer, image, { quoted: mek })
                    break
                    case 'wikipedia':
		...
		if (isBanned) return reply(ind.baned())
			...
			if (!isNsfw) return reply(ind.nsfwoff())
					...
					reply(ind.wait())
					buffer = await getBuffer(anu.result)
					Zero.sendMessage(from, buffer, image, { caption: 'Comli teross', quoted: mek})
					await limitAdd(sender)
					break			
			case 'blowjob':
			if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
                ...
                reply(ind.wait)
                   ...
                   Zero.sendMessage(from, buff, image, {quoted: mek, caption: resep})
                   await limitAdd(sender) 
                   break 
                   case 'stalkig':
                   if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
                   if (isLimit(sender)) return reply(ind.limitend(pusname))
                    ...
                    Zero.sendMessage(from, buffer, image, {quoted: mek, caption: hasil})
                    await limitAdd(sender)
			        break 
			       case 'githubstalk':
					if (!isRegistered) return reply(ind.noregis())
		...
		if (!isRegistered) return reply(ind.noregis())
                    if ((isMedia && !mek.message.videoMessage || isQuotedSticker)) {
                        if (args.length == 0) return reply(`Contoh: ${prefix + command} Zero|Ganz`)
                        const encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
                        filePath = await Zero.downloadAndSaveMediaMessage(encmedia, filename = getRandom());
                        file_name = getRandom(".webp")
                        ini_txt = args.join(" ").split("|")
		...
		if (!isRegistered) return reply(ind.noregis())
                    ...
                    Zero.sendMessage(from, ini_buffer, sticker, { quoted: mek})
                    break
                    case 'kickall':
                    if (!isOwner) return reply(ind.ownerb())
			if (!isGroupAdmins) return reply(ind.admin())
					...
					Zero.groupRemove(from, members_id)
					break 
					case 'setreply':
					if (!isOwner) return reply(ind.ownerb())
                    			Zero.updatePresence(from, Presence.composing) 
					...
					Zero.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', filename: `masyaallah.mp3`, quoted: mek })
					break
					case 'grouplist':
					if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, teks.trim(), extendedText, {quoted: mek})
					break
					case 'blocklist':
					...
					Zero.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": blocked}})
					break
		case 'daftar':
                ...
                if (isGroup) {
                    ...
                    await Zero.sendMessage(from, pp_user, image, {quoted: mek, caption: ind.registered(namaUser, umurUser, serialUser, biasalah, sender)})
		    ...
		    reply('Terima Kasih Sudah Daftar')
				...
				if (isBanned) return reply(ind.baned())   
					...
					Zero.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": registered}})
					break
            	case 'mining':
                      if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					Zero.sendMessage(from, 'Pertanyaan : *'+bisakah+'*n\nJawaban : '+ keh, text, { quoted: mek })
					await limitAdd(sender)
					break
				case 'kapankah':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					Zero.sendMessage(from, 'Pertanyaan : *'+kapankah+'*n\nJawaban : '+ koh, text, { quoted: mek })
					await limitAdd(sender)
					break
         			  case 'apakah':
         			  if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					Zero.sendMessage(from, 'Pertanyaan : *'+apakah+'*n\nJawaban : '+ kah, text, { quoted: mek })
					await limitAdd(sender)
					break
		case 'babi':
		if (!isRegistered) return reply(ind.noregis())
		...
		if (isLimit(sender)) return reply(ind.limitend(pusname))
		if (!isGroup) return reply(ind.groupo())
                    const gmek = await Zero.getGroupMembersId(groupId)
                    let gmik = gmek[Math.floor(Math.random() * gmek.length)]
                    const mmkk = `YANG PALING BABI DISINI ADALAH @${gmik.replace(/@c.us/g, '')}`
                    Zero.sendTextWithMentions(dari, mmkk, id)
                    break
                    case 'nulis2':
                 if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
				...
				Zero.sendMessage(from, buffer, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		case 'nulis3':		
                 if (!isRegistered) return reply(ind.noregis())
	...
	if (!isRegistered) return reply(ind.noregis())
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
			...
			reply(ind.wait)
				  ...
				  nama = tulis.split("|")[0];
					...
					Zero.sendMessage(from, nulis, image, {quoted: mek})
					await limitAdd(sender) 
					break  
				case 'quran':
			if (!isRegistered) return reply(ind.noregis())
	  ...
	  case 'seberapagay':
		...
		if (isBanned) return reply(ind.baned())
				...
				Zero.sendMessage(from, 'Persen Bucin Kak: *'+rate+'*n\nJawaban : '+ pbucin +'', text, { quoted: mek })
				await limitAdd(sender) 
				break 
			case 'pantun':
			if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
                if (isLimit(sender)) return reply(limitend(pushname))
					...
					Zero.sendMessage(from, pok, image, { quoted: mek , caption: 'nihhh'})
					await limitAdd(sender) 
					break  
                case 'kucing':
                if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
                if (isLimit(sender)) return reply(limitend(pushname))
					...
					Zero.sendMessage(from, pok, image, { quoted: mek , caption: '*nyang🐈*'})
					await limitAdd(sender) 
					break 
		case 'fitnah':
		if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, nye, image, { caption: 'Inu!!', quoted: mek })
					break
	case 'hobby':
           if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
           if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					Zero.sendMessage(from, 'Pertanyaan : *'+hobby+'*n\nJawaban : '+ by, text, { quoted: mek })
					await limitAdd(sender)
					break
				case 'meme':
				if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
				...
				reply(ind.wait())
					...
					case 'testime':
					setTimeout( () => {
					Zero.sendMessage(from, 'Waktu habis:v', text, {quoted: mek}) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					Zero.sendMessage(from, '5 Detik lagi', text, {quoted: mek}) // ur cods
					}, 5000) // 1000 = 1s,
					setTimeout( () => {
					Zero.sendMessage(from, '10 Detik lagi', text, {quoted: mek}) // ur cods
					}, 0) // 1000 = 1s,
					break 
		case 'caklontong':
                if (!isRegistered) return reply(ind.noregis())
		...
		if (!isGroup) return reply(ind.groupo())
					...
					setTimeout( () => {
					Zero.sendMessage(from, caklontong, text, {quoted: mek}) // ur cods
					}, 0) // 1000 = 1s,
					await limitAdd(sender) 
					break 
					case 'join':
		...
		if (!isGroup) return reply(ind.groupo())
					...
					setTimeout( () => {
					Zero.sendMessage(from, '*➸ Jawaban :* '+anu.result.jawaban, text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					Zero.sendMessage(from, '_10 Detik lagi…_', text) // ur cods
					}, 20000) // 1000 = 1s,
					...
					setTimeout( () => {
					Zero.sendMessage(from, family, text, {quoted: mek }) // ur cods
					}, 0) // 1000 = 1s,
					await limitAdd(sender) 
					break 
         	case 'ping':
          	if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
           	await Zero.sendMessage(from, `Pong!!!!\nSpeed: ${processTime(time, moment())} _Second_`)
		break
               			case 'allfiture': 
               			case 'all':
                 if (!isRegistered) return reply(ind.noregis())
...
║┃🗃️Model Device : ${device_model}
║┃🌐Browser : ${Zero.browserDescription[1]}
║┃🛰️Server : ${Zero.browserDescription[0]}
║┃🖥️Version : ${Zero.browserDescription[2]}
║┃
║┃
║┣❏ MCC : ${mcc}
║┣❏ MNC : ${mnc}
...
╚═════════════════❍
` 
                    Zero.sendMessage(from, wew, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./akiraganz/fake.jpg`)},"productId":"3937202479680283","title":"©ZEE 𝐁𝐎𝐓","currencyCode":"RGM","priceAmount1000":"999999999000","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}}, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true, caption: subscribe})
					break
					case 'help':
					case 'menu':
                 if (!isRegistered) return reply(ind.noregis())
...
├ •  *Version :* 5.2
├ •  *Prefix :* ${prefix}
├ •  *Server :* ${Zero.browserDescription[0]}
├ •  *Speed :* ${process.uptime()}
...
➤ _Jika ingin Melihat Menu Lengkap_
➤ _Ketik ${prefix}all_
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, subscribe, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./akiraganz/fake.jpg`)},"productId":"3937202479680283","title":"©ZEE 𝐁𝐎𝐓","currencyCode":"RGM","priceAmount1000":"999999999000","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}}, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
					case 'ownermenu':
					case 'ownermenu💻':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, groupmenu, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break 
			    case 'makermenu':
			    case 'makermenu🖌️':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, makermenu, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break 
		        case 'encryptmenu':
			    case 'encryptmenu🔒':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, encryptmenu, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
                case 'videomaker':
			    case 'videomaker📽️':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, videomaker, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
				case 'antivirtexmenu':
			    case 'antivirtexmenu👾':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, antivirtex, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
                case 'funmenu':
			    case 'funmenu🎰':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, funmenu, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
				case 'newsmenu':
			    case 'newsmenu📻':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, newsmenu, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
			    case 'searchmenu':
			    case 'searchmenu🔎':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, searchmenu, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
				case 'stickermenu':
			    case 'stickermenu😀':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, stickermenu, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
					case 'photoeditor':
					case 'photoeditor📸':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, photoeditor, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
					case 'soundmenu':
					case 'soundmenu🔉':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, soundmenu, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
					case 'limitmenu':
					case 'limitmenu💣':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, limitmenu, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
					case 'plastiqmenu':
					case 'plastiqmenu🗑️':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, plastiqmenu, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
					case 'mediamenu':
					case 'mediamenu🗄️':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, mediamenu, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
					case 'nsfwmenu':
					case 'nsfwmenu🔞':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, nsfwmenu, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
					case 'moviestory':
					case 'moviestory💽':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, moviestory, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
					case 'islammenu🕋':
					case 'islammenu':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, islammenu, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
					case 'animemenu':
					case 'animemenu🎌':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, animemenu, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
					case 'magermenu':
					case 'magermenu🚼':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, magermenu, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
					case 'sertifikatmenu':
					case 'sertifikatmenu🏆':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, sertifikatmenu, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
				case 'asupanmenu':
			    case 'asupanmenu🎞️':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, asupanmenu, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
				case 'linkmenu':
			    case 'linkmenu🔗':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, linkmenu, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
				case 'cloudstorage':
			    case 'cloudstorage🗃️':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, cloudstorage, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
				case 'toolsmenu':
			    case 'toolsmenu🛠️':
				if (!isRegistered) return reply(ind.noregis())
...
└──「 🖥️ZEE 𝐁𝐎𝐓🖥️ 」
`
                    Zero.sendMessage(from, toolsmenu, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*TSUNDERE*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, isBroadcast: true})
					break
				case 'donasi':
				case 'donate':
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					Zero.sendMessage(from, donasi(), text)
					break
		case 'quotesislami':
                 if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, islam, image, { quoted: mek, caption: '*Quotes Islami*n\n'+ isl })
					await limitAdd(sender)
					break	
		case 'quotesnasehat':
                 if (!isRegistered) return reply(ind.noregis())
				...
				if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, nase, image, { quoted: mek, caption: '*Quotes Nasehat*n\n'+ nsh })
					await limitAdd(sender)
					break	
					case 'otagall':
			if (!isRegistered) return reply(ind.noregis())
				if (isBanned) return reply(ind.baned())
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘  *Zero Bot* 〙', members_id, true)
					break
		case 'quotescinta':
                 if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, cta, image, { quoted: mek, caption: '*Quotes Cinta*n\n'+ cin })
					await limitAdd(sender)
					break
					case 'beban':
					limitAdd(sender) 
		...
		// Movie & Story
				...
				if (!isRegistered) return reply(ind.noregis())
					...
					Zero.sendMessage(from, 'Pertanyaan : Cek Ganteng Bang *'+ganteng+'*\n\nJawaban : '+ teng +'', text, { quoted: mek })
					break
				case 'cantikcek':
				case 'cekcantik':
				if (isBanned) return reply(ind.baned())
				if (!isRegistered) return reply(ind.noregis())
					...
					Zero.sendMessage(from, 'Pertanyaan : Cantik Cek Kakak *'+cantik+'*\n\nPersen Kecantikan : '+ tik +'', text, { quoted: mek })
					break
				  case 'ownergrup':
				  case 'ownergroup':
               Zero.updatePresence(from, Presence.composing) 
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           Zero.sendMessage(from, options, text, { quoted: mek } )
                ...
                case 'infogempa':
                    ...
                    Zero.sendMessage(from, get_buffer, image, { quoted: mek, caption: ini_txt })
                    break
                 case 'telesticker':
                ...
                case 'tiktoknowm':
                    ...
                    Zero.sendMessage(from, ini_buffer, video, { quoted: mek })
                    break
                case 'ngif':
                case 'nsfwnekogif':
                ...
                case 'tiktokmusic':
                    ...
                    Zero.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, quoted: mek })
                    break
                case 'spotify':
                    ...
                    Zero.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_result.title}.mp3`, quoted: mek })
                    break
                case 'spotifysearch':
                    if (args.length == 0) return reply(`Contoh: ${prefix + command} Melukis Senja`)
                    query = args.join(" ")
                ...
                case 'jooxplay':
                    ...
                    Zero.sendMessage(from, thumbnail, image, { quoted: mek, caption: ini_txt })
                    get_audio = await getBuffer(get_result.audio[0].link)
                    Zero.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_result.info.song}.mp3`, quoted: mek })
                    break
                case 'igdl':
                    ...
                    Zero.sendMessage(from, ini_buffer, ini_type, { quoted: mek })
                    break
                case 'fbdl':
                ...
                case 'gimage2':
                    ...
                    for (var x = 0; x <= 5; x++) {
                        ...
                        Zero.sendMessage(from, ini_buffer, image)
                    }
                    break
                case 'konachan':
                    if (!isRegistered) return reply(ind.noregis())
		...
		case 'quotesmotivasi':
				...
				if (isBanned) return reply(ind.baned())
					...
					Zero.sendMessage(from, vass, image, { quoted: mek, caption: '*Motivasi*n\n'+ cin })
					await limitAdd(sender)
					break
                case 'level':
		case 'lvl':
		...
		if (isBanned) return reply(ind.baned())
               ...
               Zero.sendMessage(from, resul, text, { quoted: mek})
                .catch(async (err) => {
                        console.error(err)
                        await reply(`Error!\n${err}`)
                    })
		...
		if (isBanned) return reply(ind.baned())
                ...
                try {
					...
					me = Zero.user
					uptime = process.uptime()
					teks = `*Nama Bot* : ${namabot}\n*Owner* : *${namaowner}*n*Author* : Zero\n*Nomor Bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total Block* : ${blocked.length}\n*Bot aktif selama* : ${kyun(uptime)}`
					buffer = await getBuffer(me.imgUrl)
					Zero.sendMessage(from, buffer, image, {caption: teks, contextInfo:{mentionedJid: [me.jid]}})
					break
				case 'blocklist': 
					teks = '𝗕𝗟𝗢𝗖𝗞 𝗟𝗜𝗦𝗧 :\n'
					for (let block of blocked) {
		...
		if (!isRegistered) return reply(ind.noregis())
                    ...
                    Zero.sendMessage(from, buffer, image, { quoted: mek, caption: 'Plastiqnya Kaka:v'  })
                    break
                    case 'blackpink':
		if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
                ...
                var gh = body.slice(12)
					...
					buffer = await getBuffer(anu.result)
					Zero.sendMessage(from, buffer, image, {caption: 'Nih anjim', quoted: mek})
					await limitAdd(sender)
					break
				case 'pornhub':
                case 'glitch':
		...
		Zero.sendMessage(from, truteh, image, { caption: '*TRUTH*n\n'+ ttrth, quoted: mek })
		break
		case 'dare':
		if (!isRegistered) return reply(ind.noregis())
		...
		Zero.sendMessage(from, tod, image, { quoted: mek, caption: '*DARE*n\n'+ der })
		break
		case 'ssweb':
                if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					Zero.sendMessage(from, buff, image, {quoted: mek})
					await limitAdd(sender)
					break
                case 'pokemon':
                if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                   ...
                   if (!isNsfw) return reply(ind.nsfwoff())
					...
					pok = await getBuffer(nimek)
					Zero.sendMessage(from, pok, image, { quoted: mek })
					await limitAdd(sender)
					break
                case 'anjing':
                if (!isRegistered) return reply(ind.noregis())
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                   ...
                   if (!isNsfw) return reply(ind.nsfwoff())
					...
					if (Number(args[0]) === 1) {
						...
						Zero.sendMessage(from,`ALLERT!!! DILARANG KIRIM VIRUS!!KARNA GRUP INI BERSIFAT ANTI VIRUS!!JIKA KIRIM VIRUS MAKA AKAN DI KICK!!`, text)
					} else if (Number(args[0]) === 0) {
					...
					Zero.sendMessage(from, { quoted: mek})
					break
					case 'antialakazam':
					if (!isGroup) return reply(ind.groupo())					
					if (!isBotGroupAdmins) return reply('BOT HARUS JADI ADMIN DULU')					
		...
		if (isLimit(sender)) return reply(ind.limitend(pushname))
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						Zero.sendMessage(from, bufferlkj, audio, {mimetype: 'audio/mp4', quoted: mek})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender) 
					break
                ...
                Zero.sendMessage(from, buffer, image, {quoted: mek, caption: ingfomp3})
                Zero.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', quoted: mek})
                break
                case 'ytmp4':
                    ini_link = args[0]
                    ...
                    Zero.sendMessage(from, get_audio, video, { mimetype: 'video/mp4', filename: `${get_result.title}.mp4`, quoted: mek })
                    break
                case 'darkneon':
                case 'candlemug':
                case 'lovemsg':
		...
		if (isBanned) return reply(ind.baned())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                    ...
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return Zero.sendMessage(from, 'Teksnya kepanjangan Bambank', text, {quoted: mek})
                    buff = await getBuffer(`https://docs-jojo.herokuapp.com/api/text3d?text=${teks}`, {method: 'get'})
                    Zero.sendMessage(from, buff, image, {quoted: mek, caption: `${teks}`})
			     	await limitAdd(sender)
					break
			    case 'fototiktok':
			if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
			if (isLimit(sender)) return reply(ind.limitend(pusname))
                ...
                Zero.sendMessage(from, buffer, image, {quoted: mek, caption: `${body.slice(5)}`})
				await limitAdd(sender)
					break
                case 'kbbi':
                if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
			...
			if (!tex) return Zero.sendMessage(from, 'MASUKAN URL/TEKS UNTUK DI JADIKAN QR', text, {quoted: mek})
		...
		if (isBanned) return reply(ind.baned())
			...
			Zero.sendMessage(from, buffer, image, {quoted: mek, caption: `${teks}`})
			await limitAdd(sender)
			break
				case 'happymod':
				if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return Zero.sendMessage(from, 'Diperlukan kode bahasa!!', text, {quoted: mek})
					const gtts = require('./akiraganz/gtts')(args[0])
					if (args.length < 2) return Zero.sendMessage(from, 'Mana teks yang ma di jadiin suara? suara saya kah:v?', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
					...
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							...
							buffer = fs.readFileSync(rano)
							if (err) return reply(ind.stikga())
							Zero.sendMessage(from, buffer, audio, {quoted: mek, ptt:true})
							fs.unlinkSync(rano)
						})
					})
					await limitAdd(sender)
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				try {
						if (args.length < 1) return Zero.sendMessage(from, '𝘂𝘀𝗲𝗿𝗻𝗮𝗺𝗲 𝗺𝗮𝗻𝗮 ?', text, {quoted: mek})
                 ...
                 case 'linkgc':
				    ...
				    Zero.sendMessage(from, yeh, text, {quoted: mek})
			        await limitAdd(sender)
					break
				case 'tagall':
					if (!isGroup) return reply(ind.groupo())
				...
				case 'clearall':
					...
					Zero.setMaxListeners(25)
					for (let _ of anu) {
						Zero.deleteChat(_.jid)
					}
					reply(ind.clears())
					break
			       case 'block':
				 ...
				 Zero.chatRead (from)
					...
					Zero.sendMessage(from, `perintah Diterima, memblokir ${body.slice(7)}@c.us`, text)
					break
                    case 'unblock':
					if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
				    Zero.blockUser (`${body.slice(9)}@c.us`, "remove")
					Zero.sendMessage(from, `Perintah diterima, Membuka ${body.slice(9)}@c.us`, text)
					break
				case 'leave': 
				if (!isGroup) return reply(ind.groupo())
				if (!isOwner) return reply(ind.ownerb())
				await reply(from, 'bye').then(() => Zero.groupLeave(from))
					break 
			case 'delete':
			case 'del':
			case 'd':
			if (!isGroup) return reply(ind.groupo())
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await Zero.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							Zero.sendMessage(_.jid, buff, image, {caption: `❮ 𝙋𝙀𝙎𝘼?? 𝘽??𝙊𝘼𝘿𝘾𝘼𝙎𝙏 ❯\n\n${body.slice(4)}`})
						}
			   	...
			   	case 'setpp': 
                       ...
                       if (!isGroupAdmins) return reply(ind.admin())
                        if (!isBotGroupAdmins) return reply(ind.badmin())
                       media = await Zero.downloadAndSaveMediaMessage(mek)
            ...
            case 'owner':
            case 'creator':
                  Zero.sendMessage(from, {displayname: "Jeff", vcard: vcard}, MessageType.contact, { quoted: mek})
                  Zero.sendMessage(from, 'Tuh kontak ${namaowner} <•_•> ,Jangan Lupa Save Ya (*>*)',MessageType.text, { quoted: mek} )
                  ini_buffer = await getBuffer("https://i.ibb.co/3RXKdX3/20210406-202806.jpg")
                  Zero.sendMessage(from, ini_buffer, MessageType.image, {quoted: mek, caption: '*SUBSCRIBE:*\n*https://m.youtube.com/channel/UCp0hD5-u2jzhHrB9ksTtb4Q*'})
                  break
		break    
           	case 'setname':
                if (!isGroup) return reply(ind.groupo())
                ...
                Zero.sendMessage(from, 'Succes, Ganti Nama Grup', text, {quoted: mek})
					break
                case 'setdesc':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
                ...
                Zero.sendMessage(from, 'Succes, Ganti Deskripsi Grup', text, {quoted: mek})
					break
           				case 'demote':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
		...
		if (isBanned) return reply(ind.baned())
				if (!isQuotedSticker) return reply('Reply/tag sticker !')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						Zero.sendMessage(from, buffer, image, {quoted: mek, caption: 'tuh dh jadi '})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
				...
				case 'clone':
					...
					try {
						pp = await Zero.getProfilePicture(id)
						buffer = await getBuffer(pp)
						Zero.updateProfilePicture(botNumber, buffer)
				...
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						reply(ind.wait())
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						media = await Zero.downloadMediaMessage(encmedia)
						await wait(media).then(res => {
							Zero.sendMessage(from, res.video, video, {quoted: mek, caption: res.teks.trim()})
						}).catch(err => {
							reply(err)
						})
					} else {
		...
		Zero.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
			...
			if (isGroup && !isCmd && isSimi && budy != undefined) {
						...
						console.log(color('[Zero Bot]','yellow'), ('Command Tidak Ditemukan!!','red'), color(sender.split('@')[0]))
					}
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
		}
